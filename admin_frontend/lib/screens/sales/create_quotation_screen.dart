      ...
      switch (field) {
        case 'description':
          _lines[index] = QuotationLine(
            id: _lines[index].id,
            quotationId: _lines[index].quotationId,
    ...
    if (_formKey.currentState?.validate() ?? false) {
      ...
      try {
        final quotationData = {
          ...
          'validUntil': _validUntilController.text.isNotEmpty ? _validUntilController.text : null,
        ...
        await ref.read(createQuotationProvider(quotationData).future);

        if (mounted) {
          Navigator.pop(context, true);
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('تم إنشاء عرض السعر بنجاح')),
          );
        }
      } catch (e) {
        if (mounted) {
          ...
          children: [
            ...
            Card(
              child: Padding(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text('إجمالي عرض السعر:', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    Text(
            ...
            ElevatedButton(
              ...
              child: _isLoading
                  ...
                  : const Text('حفظ عرض السعر', style: TextStyle(fontSize: 16)),
            ),
          ],
        ),
      ),
