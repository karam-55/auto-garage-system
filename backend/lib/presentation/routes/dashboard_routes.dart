...
import '../../infrastructure/repositories/inventory_variant_warehouse_repository_impl.dart';
...
import '../../infrastructure/repositories/employee_contract_repository_impl.dart';
...
class DashboardRoutes {
  ...
  final InventoryVariantWarehouseRepositoryImpl _inventoryRepository;
  final BillOfMaterialsRepositoryImpl _bomRepository;
  final ManufacturingOrderRepositoryImpl _manufacturingOrderRepository;
  final EmployeeContractRepositoryImpl _contractRepository;
  ...
  Future<Response> _getInventoryStats(Request request) async {
    try {
      final inventory = await _inventoryRepository.findAll();
      ...
      for (final item in inventory) {
        ...
        topMovingItems[item.inventoryVariantId] = item.quantity;
      }
  ...
  Future<Response> _getHrStats(Request request) async {
    try {
      final contracts = await _contractRepository.findAll();
      
      int totalEmployees = contracts.toSet().length;
